...
- App directory exists at `/var/www/ecombykp` and is writable by your deploy user.
- Environment file on the server (e.g., `/var/www/ecombykp/.env`) with production values.
...
- Ensures the repo remote uses the token URL:
  `https://x-access-token:${{ secrets.GIT_TOKEN }}@github.com/kakkadpriyansh/ecombykp.git`.
- Runs `git fetch` and `git reset --hard origin/main` in `/var/www/ecombykp`.
- Installs dependencies and builds:
  `npm ci --legacy-peer-deps || npm install --legacy-peer-deps && npm run build`.
- Restarts the app via PM2: `pm2 restart ecombykp --update-env` (or starts it if missing) and `pm2 save`.
...
- Permission issues on `/var/www/ecombykp`: ensure the deploy user owns the directory (`chown -R <user> /var/www/ecombykp`).
...
**ecombykp** - Your complete e-commerce solution! 🛍️